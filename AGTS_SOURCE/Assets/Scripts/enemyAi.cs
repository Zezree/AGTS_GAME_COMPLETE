...
{
    //Nastavení AI jak se hýbe jak hledá Playera aby ho šel "zabít"
	...
	void Update()
	{
		//InvokeRepeating("shoot", 0.00f, 0.001f); Toto by mělo být střílení Enemy(nepřítele) 
		movement();
		playerDetect();
	}
	...
	void movement() //Pohyb Enemy AI 
	{
		...
		if (patrol)
		{
			...
			{
				if (hit2.collider.gameObject.tag == "obstacle") //Obstacle je zeď ve hře tak aby do ní nenarazil
				{
		...
		if (pursuingPlayer) //Začne honit Playera přes mapu nebo scénu
		{
			speed = 6.0f;
	...
	void playerDetect() //Najití hráče podle view AI
    {
	...
	public void OnCollisionEnter2D(Collision2D collision) //Pokud zasáhne Enemy AI kulka zemře a tak se přídá score
	{
